class Cleaner:
    ...
    """
    A class that cleans the downloaded data from OpenFoodFacts

    Methods
    -------
    __check_products(self, downloaded_data)
        Select products that have the required data

    __format_categories_attribute_from_json_object(self, product)
        Create a cleaned list of categories

    __set_product(self, product)
        Create a list of dicts, each dict represents a cleaned product
    """
    ...
    def __init__(self, downloaded_data):
        ...
        Parameters
        -------
        downloaded_data: instance of Downloader class
        """
        ...
        self.cleaned_products_sorted = sorted(
            self.cleaned_products, key=lambda i: i["name"]
        )
    ...
    def __check_products(self, downloaded_data):
        ...
        for product in downloaded_data.products:
            if (
                ...
                and product.get("ingredients_text")
            ):
    ...
    def __format_categories_attribute_from_json_object(self, product):
        ...
        -------
        product: dict
            Dict with uncleaned data for a product
        """
    ...
    def __set_product(self, product):
        ...
        self.product_data["url"] = "https://fr.openfoodfacts.org/produit/{}".format(
            product["code"]
        )
