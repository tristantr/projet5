class Substitution:
    ...
    def launch_substitution_program(
        ...
        self.__get_products_from_a_category(
            id_of_the_product_to_substitute, selected_category_id
        )
    ...
    def __get_products_from_a_category(
        ...
        selected_category_id: int
        """
        products = self.methods_for_db.get_products_from_a_category(
    ...
    def __get_categories_from_a_product(self, id_of_the_product_to_substitute):
        ...
        id_of_the_product_to_substitute: int
        """
        categories = self.methods_for_db.get_categories_from_a_product(
            id_of_the_product_to_substitute
        )
    ...
    def __get_products_with_better_nutriscores(self, id_of_the_product_to_substitute):
        ...
        id_of_the_product_to_substitute: int
        """
        nutriscore_of_the_product_to_substitute = self.methods_for_db.get_a_product(
            id_of_the_product_to_substitute
        ).nutriscore
        ...
        for product in self.products_from_selected_category:
            ...
            if (
                nutriscores[product_nutriscore]
                < nutriscores[nutriscore_of_the_product_to_substitute]
            ):
    ...
    def __sort_products_by_number_of_common_categories(self):
        ...
        Create a dict with number of common categories for keys and products as values
            {'1': [product A, product B], '3': [product D, product Z], ...}
        """
        number_of_common_categories_keys = list(
            self.products_by_number_of_common_categories.keys()
        )
        ...
        for product in self.products_with_better_nutriscores:
            ...
            if number_of_common_categories not in number_of_common_categories_keys:
                ...
                self.products_by_number_of_common_categories[
                    number_of_common_categories
                ].append(product)
        self.sorted_number_of_common_category = sorted(
            number_of_common_categories_keys, reverse=True
        )
    ...
    def __get_a_substitute_id(self):
        ...
        if (
            self.products_by_number_of_common_categories[
                self.sorted_number_of_common_category[self.category_rank]
            ]
            != []
        ):
            self.substitute_id = self.products_by_number_of_common_categories[
                self.sorted_number_of_common_category[self.category_rank]
            ][self.substitute_rank]
        else:
